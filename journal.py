...
class TradingStrategy:
    ...
    def update_status_line(self):
        ...
        status = (
            ...
            f"Active Trades: {sum(len(self.positions[s]) for s in self.symbols)} | "
            f"Daily Profit: {sum(self.convert_to_points(self.daily_profit[s], s) for s in self.symbols):.1f} pts"
        )
    ...
    def mock_trade(self, symbol, trade_type, lot_size=0.01):
        """Execute a mock trade with real-time monitoring."""
        if symbol not in self.symbols:
            return f"Invalid symbol: {symbol}"
            
        if trade_type not in ["BUY", "SELL"]:
            return f"Invalid trade type. Must be 'BUY' or 'SELL'"
            
        try:
            # Get current market price
            current_price = self.get_current_price(symbol)
            if current_price is None:
                return "Failed to get current market price"
                
            # Create mock trade info
            trade_info = {
                'symbol': symbol,
                'type': trade_type,
            ...
            def display_trade_progress():
                try:
                    ...
                    while True:
                        ...
                        current_price = self.get_current_price(symbol)
                        if current_price is None:
                            print("\nFailed to get current price. Ending mock trade.")
                            break
                            
                        # Calculate profit in points
                        point = self.point.get(symbol, 0.0001)
                        ...
                        # Format status line
                        status = (
                            f"\r{symbol} | {trade_type} | {lot_size:.2f} lots | "
                            f"Entry: {trade_info['entry_price']:.5f} | "
                            f"Current: {current_price:.5f} | "
                            f"P/L: {profit_points:.1f} pts | "
                    ...
                    print("\nMock trade terminated by user")
                    trade_info['status'] = 'CLOSED'
                    trade_info['close_time'] = datetime.now()
                    trade_info['close_price'] = self.get_current_price(symbol)
                    return
                    
            # Start the progress display in the current thread
            display_trade_progress()
            return "Mock trade completed"
